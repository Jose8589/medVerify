...
(function () {
  ...
  function onScroll() {
    ...
    // dock appears after hero, hides over the final form section
    if (dock) {
  ...
  if (consoleEl) {
    ...
    function resolveRow(row) {
      var status = row.getAttribute("data-status");      // ok | warn
  ...
  document.querySelectorAll("form[data-intake]").forEach(function (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (!data.doctor_name && !data.clinic_name) {
        form.querySelector(".field__input").focus();
        return;
      }
      // Replace with backend that creates a Stripe Checkout Session and
      // returns its url; pass `data` as Checkout metadata.
